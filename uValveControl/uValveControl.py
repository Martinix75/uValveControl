...
#Wdt = WDT(timeout=5000) #attiva il watch dog

Versione_ = '0.3 Alpha1'

def Scrivi_Configurazione():
    martinLib.Write_Config(Ton=Dati.Ton, Toff=Dati.Toff, T_Press=Dati.T_Press)
    
def Leggi_Configurazione(**Parametri):
    valori = martinLib.Read_Config(**Parametri)
    Dati.Ton = valori['Ton']
    Dati.Toff = valori['Toff']
    Dati.T_Press = valori['T_Press']

def test(): #funzione test per i led (li accende tutti)
    Valvola.high()
    Led_Rem.high()
    Led_Loc.high()
    Led_Alr.high()
...
'''
test()
Leggi_Configurazione(Ton=Dati.Ton, Toff=Dati.Toff, T_Press=Dati.T_Press)
Apri = martinLib.Holding_Pulse(P_Apri, Dati.T_Press)
Chiudi = martinLib.Holding_Pulse(P_Chiudi, Dati.T_Press)
#uasyncio.run(main(1))
while True:
    #Wdt.feed() #resetta conteccio watc dog
